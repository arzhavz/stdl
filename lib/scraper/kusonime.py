...
def KusonimeSearch(query: str = None) -> dict:
	results = []
	res_tup = []
	try:
		url = f"https://kusonime.com/?s={quote(query)}&post_type=post"
		data = requests.get(url)
		html = soup(data.text, "html.parser")
		venz = html.find("div", {"class": "venz"})
		kover = venz.find_all("div", {"class": "kover"})
		for anim in kover:
			results.append({
				"title": anim.find("a").get("title"),
				"url": anim.find("a").get("href")
			})
			res_tup.append((anim.find("a").get("href"), anim.find("a").get("title")))
		return res_tup
	except Exception as e:
		raise e